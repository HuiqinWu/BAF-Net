...
class weighted_ResNet_early(nn.Module):
    def __init__(self, in_channel=4):
        super(weighted_ResNet_early, self).__init__()
    ...
    def forward(self, pet, ct):
        pet = pet * self.pet_weight
        ct = ct * self.ct_weight

        pet_ct = torch.cat((pet, ct), dim=1)
